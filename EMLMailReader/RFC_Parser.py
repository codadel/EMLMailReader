...
class StandardsParser:
    ...
    def _native_children(self, result: RxMailMessage, part: Message) -> list[Message]:
        ...
        if media_type != "message/global" or transfer_encoding not in ("base64", "quoted-printable"):
            return children
        try:
    ...
    def _headers(self, message: Message) -> HeaderCollection:
        ...
        for index, (raw_name, raw_value) in enumerate(raw_items):
            parsed_value = str(parsed_items[index][1]) if index < len(parsed_items) else raw_value
            unfolded = re.sub(r"(?:\r\n|\n|\r)(?=[ \t])", "", raw_value)
            status = SyntaxStatus.OBSOLETE if (
                re.search(r"(?:\r\n|\n|\r)[ \t]+(?:\r\n|\n|\r)", raw_value)
    ...
    def _defects(self, message: Message) -> list[ParseDiagnostic]:
        ...
        for defect in getattr(message, "defects", ()):
            diagnostics.append(ParseDiagnostic(
                ...
                severity=DiagnosticSeverity.ERROR if self.parsing_mode == ParsingMode.STRICT else DiagnosticSeverity.WARNING,
                rfc="RFC 5322/MIME",
            ))
        return diagnostics
    ...
    def _populate_imf(self, result: RxMailMessage, message: Message) -> None:
        ...
        result.InReplyTo = self._parse_message_ids(result.Headers.get("In-Reply-To", ""))
        result.References = self._parse_message_ids(result.Headers.get("References", ""))
    ...
    def _validate_mime_entity(self, result: RxMailMessage, part: Message, media_type: str) -> None:
        ...
        if media_type == "message/partial":
            ...
            if not metadata.id or metadata.number is None or metadata.number < 1:
                result.Diagnostics.append(ParseDiagnostic(
        ...
        if media_type == "message/external-body" and not result.ExternalBodyAccess.access_type:
            result.Diagnostics.append(ParseDiagnostic(
        ...
        if restricted_composite and result.ContentTransferEncoding.kind in (
            TransferEncoding.BASE64, TransferEncoding.QUOTED_PRINTABLE
        ):
            result.Diagnostics.append(ParseDiagnostic(
        ...
        if explicit and explicit.MediaType.startswith("text/") and "charset" not in explicit.Parameters:
            result.Diagnostics.append(ParseDiagnostic(
    ...
    def _populate_leaf_body(self, result: RxMailMessage, part: Message, top_level: bool = False) -> None:
        ...
        if decoded is None:
            payload = part.get_payload()
            decoded = payload.encode("utf-8", "surrogateescape") if isinstance(payload, str) else b""
    ...
    def _validate_required(self, result: RxMailMessage) -> None:
        ...
        if result.Headers.get_all("Date") and (result.Date is None or not result.Date.valid):
            result.Diagnostics.append(ParseDiagnostic(
                code="InvalidDate",
        ...
        if result.Headers.get_all("Message-ID") and (result.MessageID is None or not result.MessageID.valid):
            result.Diagnostics.append(ParseDiagnostic(
        ...
        has_mime_fields = any(result.Headers.get_all(name) for name in _MIME_SINGLETON_HEADERS)
        ...
        if result.MimeVersion and not re.fullmatch(r"\s*\d+\s*\.\s*\d+\s*", mime_version):
            result.Diagnostics.append(ParseDiagnostic(
        ...
        for name in ("From", "Sender"):
            ...
            if address_list and any(isinstance(item, AddressGroup) for item in address_list.Items):
                result.Diagnostics.append(ParseDiagnostic(
    ...
    def _validate_resent_blocks(result: RxMailMessage) -> None:
        for index, block in enumerate(result.ResentBlocks, start=1):
            ...
            if from_values:
                ...
                if len(addresses.Mailboxes) > 1 and not block.fields.get_all("Resent-Sender"):
                    result.Diagnostics.append(ParseDiagnostic(
    ...
    def _source_diagnostics(self, source: bytes, headers: HeaderCollection) -> list[ParseDiagnostic]:
        ...
        for line_number, line in enumerate(re.split(br"\r\n|\n|\r", source), start=1):
            if len(line) > 998:
                diagnostics.append(ParseDiagnostic(
                    code="LineTooLong",
    ...
    def _parse_date(raw: str) -> ParsedDateTime | None:
        ...
        try:
            obsolete = bool(
                ...
                or re.search(r"\b(?:UT|GMT|EST|EDT|CST|CDT|MST|MDT|PST|PDT|[A-IK-Z])\b", raw, re.I)
            )
    ...
    def _parse_message_id(raw: str) -> ParsedMessageID | None:
        ...
        return ParsedMessageID(raw, f"{match.group(1)}@{match.group(2)}", match.group(1), match.group(2), True)
