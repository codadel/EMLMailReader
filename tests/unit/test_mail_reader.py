...
class TestMailReader(unittest.TestCase):
    ...
    def test_bytes_string_and_stream_entry_points(self):
        ...
        self.assertEqual(reader.parse_stream(io.StringIO(self.SOURCE.decode())).Body, "body")

    def test_constructor_configures_requested_logging_mode(self):
    ...
    def test_nested_diagnostics_are_logged(self):
        source = (
            self.SOURCE.split(b"\r\n\r\n")[0]
        ...
        self.assertTrue(any("DuplicateMIMEHeader" in call.args[0] for call in logentry.call_args_list))
