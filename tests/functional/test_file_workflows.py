...
def test_get_email_reads_an_eml_file():
...
def test_get_email_returns_none_for_missing_empty_and_read_error(tmp_path):
...
def test_get_email_closes_file_after_parse_error(tmp_path):
...
def test_strict_file_parse_propagates_compliance_errors():
...
def test_attachment_save_sanitizes_name_and_writes_decoded_bytes(tmp_path):
...
def test_unnamed_attachment_save_uses_fallback_name(tmp_path):
...
def test_attachment_save_requires_an_existing_folder(tmp_path):
